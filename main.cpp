...
static WEBXM* webx_init()
{
	WEBXM* boot = new WEBXM();

	printf("[webx 2.0] Initializing webx master server socket...\n");

	boot->HTTP_init.address			= "0.0.0.0";
	boot->HTTP_init.port			= "80";
	boot->HTTP_init.dataprotocol	= CSOCKET_TCP;
	boot->HTTP_init.ipprotocol		= CSOCKET_IPV4;
	boot->HTTP_init.secure			= CSOCKET_HTTP;
	...
	boot->http_queue  = new HTTPQueue();
	boot->https_queue = new HTTPQueue();

	boot->httph = new HTTPHandler();
...
int __cdecl main()
{
	printf("[webx 2.0] webx boot procedure executed...\n");

	webx = webx_init();
	if (webx == nullptr) {
		printf("[webx 2.0] framework boot procedure failed, closing now\n");
	...
	//register all our request handlers
	printf("[webx 2.0] Registering request handlers...\n");
	//
	//

	//register all our web C func handlers
	printf("[webx 2.0] Registering request handlers...\n");
	//
	//
	...
	while (true)
	{
		if (!webx->https_srv->IsValid()) {
			...
			if (webx->https_srv->Bind() == CSOCKET_SOCK_SUCCESS) {
				if (webx->https_srv->Listen() == CSOCKET_SOCK_SUCCESS) {
		...
		if (webx->https_srv->SelectReadable({ 0,0 }) > 0) {
			CSOCKET* client = webx->https_srv->Accept();

			if (client->IsValid()) {
				client->SSL_Init(CERT_FILE, KEY_FILE);
				client->SSLBindSocket();
...
DWORD WINAPI http_listen(LPVOID pParam)
{
	WEBXM* srv = (WEBXM*)pParam;

	while (true)
	{
		if (!srv->http_srv->IsValid()) {
			...
			if (srv->http_srv->Bind() == CSOCKET_SOCK_SUCCESS) {
				if (srv->http_srv->Listen() == CSOCKET_SOCK_SUCCESS) {
...
DWORD WINAPI cl_proc(LPVOID pParam)
{
	cl_http_packet* client = (cl_http_packet*)pParam;

	if (!client->client->IsValid()) {
		printf("[webx] connection corrupt, closing..\n");
	...
	if (client->secure) {
		client->client->SSL_Init(CERT_FILE, KEY_FILE);
		client->client->SSLBindSocket();

		if (client->client->SSLAccept() != CSOCKET_SSL_SUCCESS) {
			printf("xxx");
			delete client;
	...
	while (!shut)
	{
		...
		for (auto& n : dets/*client->http_headers*/)
		{
...
/********************************************************************
utility funcs to make my life easier
********************************************************************/
